...
function showAuth(mode = "login") {
  ...
  requestAnimationFrame(() => authSection.scrollIntoView({ behavior: "smooth", block: "center" }));
}
...
function showAuthFromHashOrLanding() {
...
async function loadCustomers() {
  ...
  try {
    ...
    state.customerTotalPages = Math.max(data.pagination.totalPages || 1, 1);
...
async function pauseCustomer(customerId) {
  const pauseStart = window.prompt("Pause start date (YYYY-MM-DD):", new Date().toISOString().slice(0, 10));
  if (!pauseStart) return;

  try {
    await api(`/api/customers/${customerId}/pause`, {
      method: "POST",
      body: JSON.stringify({ pauseStart })
    });
    await refreshDashboard();
  } catch (error) {
...
async function resumeCustomer(customerId) {
  const resumeDate = window.prompt("Resume date (YYYY-MM-DD):", new Date().toISOString().slice(0, 10));
  if (!resumeDate) return;

  try {
    await api(`/api/customers/${customerId}/resume`, {
      method: "POST",
...
function bindEvents() {
  ...
  $("#customerOrder")?.addEventListener("change", () => {
    state.customerPage = 1;
    loadCustomers();
  });
  ...
  $("#customerNext")?.addEventListener("click", () => {
    if (state.customerPage < state.customerTotalPages) {
      state.customerPage += 1;
      loadCustomers();
    }
  });

  $("#billMonth")?.addEventListener("change", (event) => {
    state.billMonth = event.target.value;
    loadBills();
  });
  ...
  $("#subscriptionForm")?.addEventListener("submit", (event) => {
    event.preventDefault();
    createSubscription(event.currentTarget);
  });
...
window.addEventListener("hashchange", () => {
  if (window.location.hash === "#auth" && !state.token) showAuth("login");
});
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    console.error("TiffinTrack frontend error:", error);
    showAuth("login");
    setMessage($("#authMessage"), "The application loaded with an unexpected frontend error. Refresh and try again.", "error");
  }
});
